function savestateClickSuppressor(event) {
    if (event.target.tagName === "A") {
        let isSavestate = event.target.href?.startsWith("blob:")
                       && event.target?.download?.endsWith("_moveplanner.json");
        let isModified = event.target?.href?.endsWith("#awbw_enhancements_modified");
        console.log("Savestate click suppressor observed: isSavestate:", isSavestate, "isModified:", isModified);
        if (isSavestate && !isModified) {
            console.log("Detected click on unmodified savestate, preventing download!");
...
class SavestateInterceptor {
    constructor(options, loadStateInput, stateSources) {
        ...
        document.body.addEventListener("click", savestateClickSuppressor);

        this.loadStateInput.addEventListener("change", this.onSavestateUploaded.bind(this));
    }

    addOnUploadListener(listener) {
        this.onUploadListeners.push(listener);
    }
    ...
    publishExtraSavestateData(extraData) {
        ...
        for (let state of extraData.states) {
            ...
            if (source) {
                ...
                reportError("Found savestate data without handler:", state);
            }
        }
    }

    restoreSavestate(savestateData) {
        this.publishExtraSavestateData(savestateData.awbw_enhancements_extras);
    }
    ...
    onSavestateUploaded() {
        ...
        reader.onload = (event) => {
            ...
            for (let listener of this.onUploadListeners) {
                listener(filename, savestateData);
            }
    ...
    onSavestateDownloadIntercepted(message) {
        ...
        let downloadModifiedSavestate = (message) => {
            ...
            .then((savestateData) => {
                ...
                savestateData.awbw_enhancements_extras = this.assembleExtraSavestateData();
                ...
                a.href = url + "#awbw_enhancements_modified";
