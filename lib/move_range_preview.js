...
class MoveRangePreview {
    ...
    showAttackRangeFor(unit, selected) {
        this.displayingSelectedUnit = selected;
        this.displayAttackRangeTiles(this.getIndirectAttackRange(unit));
    }
    ...
    getIndirectAttackRange(unit) {
        let unitData = kUnitsByName[unit.unit];
        // TODO: handle max, grit, and jake indirect range modifiers
        let playerInfo = this.playersByCountry[unit.country.code];

        let [minRange, maxRange] = unitData.range;
        if (minRange > 1) {
        ...
        for (let positionId in movePositions) {
            ...
            // Can't attack from a square that is occupied
            ...
            for (let neighborId in neighbors) {
                ...
                // TODO: check whether the unit has a weapon that can attack the target
                // */
