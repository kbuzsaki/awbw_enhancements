...
class MoveRangePreview {
    ...
    onClick(event) {
        if (event.target.id === "move") {
            ...
            if (this.rangeTiles.length > 0) {
                this.clearMoveRange();
            } else {
                let unitSpan = event.target.parentNode;
                this.showMoveRangeFor(this.getUnitWithSpan(unitSpan));
            }
        } else {
    ...
    showMoveRangeFor(unit) {
        this.displayMoveRangeTiles(this.getPositionsInRange(unit));
    }
